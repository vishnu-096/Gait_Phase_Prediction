...
def get_train_data_from_df(all_data, test_ratio):
    ...
    source_table = source_table.drop(["lgrf", "rgrf", "l_ph_ank", "r_ph_ank","l_ph_fo","r_ph_fo", 'strike_frame'], axis = 1)
    source_table
    x = source_table
...
subject_names = ['VP','AK', 'SD','PH','JL','JS', 'PK', 'SOE', 'YC', 'TH','SKS', 'VN']#,'VN','AK' 'SOE'
sub_comb_list=[]
test_sub_list=[]
acc_list=[]
rmse_list=[]
...
if os.path.exists(pkl_file):
    ...
    for file_name in file_names:
        ...
        tmp = tmp.drop(columns=['perc', 'st_sw_phase', 'strike_frame', 'lhip_ang', 'rhip_ang','l_ph_hip','r_ph_hip', 'st_l'])
